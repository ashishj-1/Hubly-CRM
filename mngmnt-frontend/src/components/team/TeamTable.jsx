...
const TeamTable = ({
  ...
  // Normalizers
  const norm = useCallback((v) => (v == null ? "" : String(v).trim()), []);
  ...
  const sortedMembers = useMemo(() => {
    const arr = [...members];
    arr.sort((a, b) => {
      const aFirst = normLower(a.firstName);
      const bFirst = normLower(b.firstName);
      if (aFirst !== bFirst) {
        return sortOrder === "asc"
          ? aFirst.localeCompare(bFirst)
          : bFirst.localeCompare(aFirst);
      }
      const aLast = normLower(a.lastName);
      const bLast = normLower(b.lastName);
      return sortOrder === "asc"
        ? aLast.localeCompare(bLast)
        : bLast.localeCompare(aLast);
    });
    return arr;
  }, [members, sortOrder, normLower]);
